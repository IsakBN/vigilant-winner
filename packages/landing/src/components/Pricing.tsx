...
/** App dashboard base URL for cross-subdomain links */
const APP_BASE_URL = 'https://app.bundlenudge.com'
...
function PricingCard({ plan }: { plan: PricingPlan }) {
  ...
  const getCTALink = () => {
    if (plan.name === 'free') {
      return `${APP_BASE_URL}/sign-up`
    }
    if (plan.name === 'enterprise') {
      return `mailto:enterprise@bundlenudge.com?subject=Enterprise%20Plan%20Inquiry`
    }
    return `${APP_BASE_URL}/sign-up?redirect_url=/dashboard/billing?upgrade=${plan.id}`
  }
