...
/**
 * Safely attempt to require a module, returning null if not available.
 */
...
function getSentry(): SentryLike | null {
  ...
  const required = tryRequire('@sentry/react-native')
...
function getBugsnag(): BugsnagLike | null {
  ...
  const required = tryRequire('@bugsnag/react-native')
  if (required && typeof required === 'object') {
...
function getCrashlytics(): CrashlyticsLike | null {
  const crashlyticsModule = tryRequire('@react-native-firebase/crashlytics')
