...
function validateProps(props: BundleNudgePluginProps): void {
  if (!props.appId || typeof props.appId !== 'string') {
    throw new Error(
      '[BundleNudge] appId is required. ' +
        'Get your app ID from the BundleNudge dashboard.',
    )
  }
 ...
 */
const withBundleNudge: ConfigPlugin<BundleNudgePluginProps> = (
  config,
  props,
) => {
 ...
 * multiple times.
 */
const bundleNudgePlugin = createRunOncePlugin(
  withBundleNudge,
  PACKAGE_NAME,
  PLUGIN_VERSION,
)
