...
describe('upload/client', () => {
  ...
  function mockStatus(status: UploadJobStatus, progress = 0, extra = {}): void {
    mockFetch.mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ status, progress, ...extra }) })
  }
  ...
  it('upload sends POST with correct headers', async () => {
    mockUploadSuccess()
    mockStatus('completed', 100)
    await new UploadClient(testConfig).upload(bundle)

    const [url, opts] = mockFetch.mock.calls[0] as [string, RequestInit]
    expect(url).toBe('https://api.bundlenudge.com/v1/uploads')
    expect(opts.method).toBe('POST')
    expect(opts.headers).toMatchObject({ Authorization: 'Bearer test-api-key', 'X-App-Id': 'test-app-id' })
  })

  it('upload polls until completed', async () => {
    mockUploadSuccess()
  ...
  it('upload respects maxAttempts', async () => {
    mockUploadSuccess()
    mockFetch.mockResolvedValue({ ok: true, json: () => Promise.resolve({ status: 'processing', progress: 50 }) })

    const resultPromise = new UploadClient(testConfig).upload(bundle, { pollInterval: 100, maxAttempts: 3 })
  ...
  it('upload calls callbacks', async () => {
    ...
    const onProgress = vi.fn(), onStatusChange = vi.fn()
    const resultPromise = new UploadClient(testConfig).upload(bundle, { pollInterval: 100, onProgress, onStatusChange })
    await vi.advanceTimersByTimeAsync(100)
    await resultPromise

    expect(onProgress).toHaveBeenCalledWith(100)
    expect(onStatusChange.mock.calls.map((c) => c[0] as UploadJobStatus)).toContain('completed')
  })

  it('getStatus fetches job status', async () => {
    mockFetch.mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ status: 'processing', progress: 50, releaseId: 'rel-123' }) })
    const result = await new UploadClient(testConfig).getStatus('job-123')

    expect(mockFetch).toHaveBeenCalledWith('https://api.bundlenudge.com/v1/uploads/job-123', expect.objectContaining({ method: 'GET' }))
  ...
  it('cancel sends DELETE request', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('https://api.bundlenudge.com/v1/uploads/job-123', expect.objectContaining({ method: 'DELETE' }))
  })
  ...
  it('uploadBundle convenience function works', async () => {
    ...
    expect(await resultPromise).toMatchObject({ jobId: 'job-456', status: 'completed' })
  })

  it('handles upload failures', async () => {
    mockFetch.mockResolvedValueOnce({ ok: false, status: 500 })
    expect(await new UploadClient(testConfig).upload(bundle)).toMatchObject({ status: 'failed', error: 'Failed to initiate upload', jobId: '' })

    mockUploadSuccess()
    mockStatus('cancelled', 0)
    expect((await new UploadClient(testConfig).upload(bundle)).status).toBe('cancelled')
  })
})
