...
import type { UploadResult, UploadOptions, UploadConfig, UploadJobStatus, UploadResponse, StatusResponse } from './types'

const DEFAULT_POLL_INTERVAL = 2000
const DEFAULT_MAX_ATTEMPTS = 60

const sleep = (ms: number): Promise<void> => new Promise((r) => setTimeout(r, ms))
const isTerminal = (s: UploadJobStatus): boolean => s === 'completed' || s === 'failed' || s === 'cancelled'
const failResult = (jobId: string, error: string): UploadResult => ({ jobId, status: 'failed', error, progress: 0 })

export class UploadClient {
  constructor(private config: UploadConfig) {}

  async upload(bundle: ArrayBuffer | Uint8Array, options: UploadOptions = {}): Promise<UploadResult> {
  ...
  async getStatus(jobId: string): Promise<UploadResult> {
    try {
      const response = await fetch(`${this.config.apiUrl}/v1/uploads/${jobId}`, {
        method: 'GET',
      ...
      return { jobId, status: data.status, releaseId: data.releaseId, error: data.error, progress: data.progress }
    } catch (error) {
  ...
  async cancel(jobId: string): Promise<void> {
    await fetch(`${this.config.apiUrl}/v1/uploads/${jobId}`, { method: 'DELETE', headers: this.buildHeaders() })
  }

  private buildHeaders(): Record<string, string> {
    return { Authorization: `Bearer ${this.config.apiKey}`, 'X-App-Id': this.config.appId }
  }

  private async initiateUpload(bundle: ArrayBuffer | Uint8Array): Promise<string | null> {
    try {
      const response = await fetch(`${this.config.apiUrl}/v1/uploads`, {
        method: 'POST',
