...
export function setDebugEnabled(enabled: boolean): void { debugEnabled = enabled }
export function isDebugEnabled(): boolean { return debugEnabled }

function addToHistory(level: LogLevel, message: string, context?: Record<string, unknown>): LogEntry {
...
function emitLog(entry: LogEntry): void {
  ...
  const methods = { debug: console.debug, info: console.info, warn: console.warn, error: console.error }
...
/** Clear log history */
export function clearLogs(): void { logHistory.length = 0 }
