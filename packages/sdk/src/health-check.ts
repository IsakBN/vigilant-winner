...
export const DEFAULT_RETRY_DELAY_MS = 5_000 // 5 seconds
...
export class HealthCheckService {
  ...
  async verifyHealth(
    checkConfig: EndpointHealthCheckConfig
    ...
    for (const endpoint of checkConfig.endpoints) {
      const result = await this.checkEndpointWithRetries(
        endpoint,
        retryCount,
        retryDelayMs
      )
  ...
  private async sendReport(
    ...
    await fetch(`${this.config.apiUrl}/v1/health/endpoint-check`, {
      method: 'POST',
      headers: {
