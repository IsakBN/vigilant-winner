...
describe('health-config', () => {
  ...
  const createFetcher = (
    overrides?: Partial<HealthConfigFetcherConfig>
  ): HealthConfigFetcher =>
    createHealthConfigFetcher({
  ...
  describe('fetchConfig - successful fetch', () => {
    ...
    it('includes authorization header when token available', async () => {
      ...
      const fetcher = createFetcher({ getAccessToken: () => 'my-secret-token' })
      await fetcher.fetchConfig()
    ...
    it('excludes authorization header when no token', async () => {
      ...
      const calls = mockFetch.mock.calls as [string, { headers: Record<string, string | undefined> }][]
      const [, options] = calls[0]
  ...
  describe('fetchConfig - config parsing', () => {
    it('returns config with empty arrays', async () => {
    ...
    it('accepts all valid HTTP methods', async () => {
      const configWithAllMethods: HealthConfig = {
        ...
        endpoints: [
          ...
          { method: 'POST', url: '/post', expectedStatus: [201], required: true },
          { method: 'PUT', url: '/put', expectedStatus: [200], required: true },
          { method: 'DELETE', url: '/delete', expectedStatus: [204], required: true },
        ],
      }
      ...
      mockFetch.mockResolvedValue({
        ok: true,
        status: 200,
        json: () => Promise.resolve(configWithAllMethods),
      })
