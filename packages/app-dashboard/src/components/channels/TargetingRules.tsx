...
import type {
    ...
    ChannelTargetingRules as TargetingRulesType,
} from '@/lib/api/types'
...
interface TargetingRulesProps {
    value: TargetingRulesType | null
    onChange: (rules: TargetingRulesType | null) => void
    disabled?: boolean
}
...
function RuleRow({ rule, index, onChange, onRemove, disabled }: RuleRowProps) {
    ...
    const displayValue = typeof rule.value === 'string' ? rule.value : String(rule.value)
...
export function TargetingRules({ value, onChange, disabled }: TargetingRulesProps) {
    ...
    const handleAddRule = () => {
        const newRule: ChannelTargetingRule = {
            field: 'appVersion',
            op: 'semver_gte',
            value: '',
        }
