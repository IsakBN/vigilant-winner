...
/**
 * BuildLogs Component - Displays build logs with auto-scroll
 */
...
function LogEntry({ log, showLevel = true }: { log: BuildLog; showLevel?: boolean }) {
    const config = LOG_LEVEL_CONFIG[log.level]
    const Icon = config.icon

    return (
        <div className={cn('flex items-start gap-2 py-1', config.className)}>
            <span className="flex-shrink-0 w-20 font-mono text-xs text-neutral-400">
        ...
        <div className="h-[400px] bg-neutral-900 rounded-lg p-4 space-y-2">
            {Array.from({ length: 10 }).map((_, i) => (
                <div key={i} className="flex items-center gap-2">
                    ...
                    <Skeleton className="h-4 bg-neutral-800" style={{ width: `${40 + i * 5}%` }} />
                </div>
            ))}
        </div>
    )
...
export function BuildLogsCompact({
    logs,
    maxLines = 5,
    className,
}: {
    ...
    return (
        <div className={cn('bg-neutral-900 text-neutral-100 rounded p-3 font-mono text-xs', className)}>
